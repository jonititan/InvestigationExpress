...
def create_pipeline(cam_list):
    ...
    for c in cam_list:
        ...
        if c == 'rgb':
            ...
            cam[c].setResolution(dai.ColorCameraProperties.SensorResolution.THE_4_K)
...
with contextlib.ExitStack() as stack:
    ...
    for device_info in device_infos:
        ...
        for cam in cam_list:
            ...
            Path(f"{dirName}/{cam}_{device.getMxId()}").mkdir(parents=True, exist_ok=True)
    ...
    while True:
        for q in queues:
            ...
            if new_msg is not None:
                ...
                tmp_timestamp = new_msg.getTimestamp()
                if check_sync(queues, tmp_timestamp):
                    for q in queues:
                        frame = q['msgs'].pop(0).getCvFrame()
                        cv2.imshow(f"{q['cam']} - {q['mx']}", frame)
                        cv2.imwrite(f"{dirName}/{q['cam']}_{q['mx']}/{(tmp_timestamp.total_seconds()-tstart)}.jpeg",frame)
